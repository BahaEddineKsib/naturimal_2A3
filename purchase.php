...
if ( isset($_GET['add_complaint_id'])){
  $panier->add_complaint($_GET['add_complaint_id'],$db);
}
?>
<!DOCTYPE html>
<html lang="en">
    			...
    			<div class="col-md-12 ftco-animate">
    				<div class="cart-list">
	    				<table class="table">
                            ...
                            try{
                                $req = $db->prepare('SELECT * FROM COMANDE WHERE email =:em AND id_commande=:id');
                                $req->bindValue(':em', $email );
                                $req->bindValue(':id', $id_co->id_commande );
                                $req->execute();
                            ...
                            $count=1;
                            while ( $count <= 6 ){
                                if ( $qts_produit[$count] == 0){
                                    unset($qts_produit[$count]);
                                }
                                $count++;
                            }
                            ...
                            // print_r($current_ids);
                            $purchases = $panier->product_table($db, $ids_produit);	
                            $i=1;
                            $tot=0;
							foreach($purchases as $product):
                                $tot += $product->price * $qts_produit[$product->id];
							 ?>
							<div class="itemsspecial">
						      <tr class="text-center">
						        ...
						        <td class="producttotal"><?= number_format($product->price * $qts_produit[$product->id] ,2,',',' ');?> $</td>
